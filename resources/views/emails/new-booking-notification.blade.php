...
<head>
    ...
    <title>{{ __('New Booking Received') }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>{{ __('New Booking Received!') }}</h1>
        <p>{{ __('You have a new booking request') }}</p>
    </div>
    ...
    <div class="content">
        ...
        <div class="highlight">
            <p style="margin: 0; font-weight: 600; color: #92400e;">{{ __('🎉 You have a new booking that requires your confirmation!') }}</p>
        </div>

        <p>{{ __('A customer has booked your service. Please review the details below and confirm or decline the booking.') }}</p>

        <div class="booking-details">
            <h3 style="margin-top: 0; color: #111827;">{{ __('Customer Information') }}</h3>
        ...
        <div class="booking-details" style="background: #eff6ff;">
            <h3 style="margin-top: 0; color: #1e40af;">{{ __('Customer Notes') }}</h3>
            <p style="color: #1e40af; margin: 0;">{{ $booking->customer_notes }}</p>
        </div>
        @endif

        <div style="text-align: center;">
            <a href="{{ route('vendor.bookings.show', $booking->id) }}" class="button">{{ __('View & Manage Booking') }}</a>
        </div>

        <div style="margin-top: 30px; padding: 15px; background: #f3f4f6; border-radius: 8px;">
            <p style="margin: 0; font-size: 14px; color: #4b5563;">
                <strong>{{ __('Quick Actions:') }}</strong><br>
                {{ __('• Confirm the booking to secure the appointment') }}<br>
                {{ __('• Contact the customer if you need more information') }}<br>
                {{ __('• Decline if you\'re not available') }}
            </p>
        </div>

        <div class="footer">
            <p>{{ __('This is an automated notification from your LocalServices vendor dashboard.') }}</p>
