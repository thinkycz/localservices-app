...
<template>
    <GuestLayout>
        ...
        <div class="mb-8 text-center">
            ...
            <h2 class="text-2xl font-bold text-gray-900">{{ $t('Verify your email') }}</h2>
            <p class="text-sm text-gray-500 mt-2 leading-relaxed max-w-sm mx-auto">{{ $t('Thanks for signing up! Please click the verification link we sent to your email address to get started.') }}</p>
        </div>
        ...
        <div v-if="verificationLinkSent" class="mb-6 flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-sm font-medium px-4 py-3 rounded-xl">
            ...
            </svg>{{ $t('A new verification link has been sent to your email address.') }}</div>
        ...
        <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-6 flex items-start gap-3">
            ...
            <p class="text-sm text-blue-700">{{ $t('Didn\'t receive the email? Check your spam folder or click below to resend.') }}</p>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="text-center">
                ...
                >{{ $t('Log out and use a different account') }}</Link>
            </div>
        </form>
    </GuestLayout>
</template>
