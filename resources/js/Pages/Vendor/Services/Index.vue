...
<template>
    ...
    <VendorLayout activePage="services">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <template v-if="services.data.length > 0">
                ...
                >
                    ...
                    <div class="h-40 bg-gray-100 relative shrink-0">
                        ...
                        />
                        <div v-else class="w-full h-full flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-50">
                        ...
                        <div class="absolute top-3 left-3 right-3 flex justify-between items-start">
                            <button
                                ...
                                :class="[
                                    'px-2.5 py-1 text-xs font-bold rounded-lg shadow-sm backdrop-blur-sm transition-colors',
                            ...
                            <span
                                v-if="service.badge"
                                :class="[
                                    getBadgeClasses(service.badge_color), 
                                    'text-[10px] font-bold px-2 py-0.5 rounded shadow-sm border border-white/20 uppercase tracking-wider backdrop-blur-sm'
                                ]"
                            >
                                {{ service.badge }}
                            </span>
                    ...
                    <div class="p-5 flex-1 flex flex-col min-w-0">
                        <div class="mb-3">
                            <div class="flex items-center justify-between gap-2 mb-1">
                                <span class="text-xs font-medium text-gray-500 truncate">{{ service.category?.name || 'Uncategorized' }}</span>
                                <span class="text-[10px] text-gray-400 shrink-0">{{ formatDate(service.created_at) }}</span>
                            </div>
                            <h3 class="font-bold text-gray-900 text-lg leading-tight mb-1 truncate" :title="service.name">{{ service.name }}</h3>
                            <p class="text-xs text-gray-500 line-clamp-2" :title="service.description">{{ service.description }}</p>
                        </div>

                        <!-- Stats Strip -->
                        <div class="mt-auto bg-gray-50 rounded-xl p-3 flex items-center justify-between mb-4">
                            <div>
                                <div class="text-[10px] sm:text-xs text-gray-500 font-medium mb-0.5">Offerings</div>
                            ...
                            <div class="text-right">
                                <div class="text-[10px] sm:text-xs text-gray-500 font-medium mb-0.5">Starting at</div>
                        ...
                        <div class="flex items-center gap-2 pt-1">
                            <Link
                                ...
                                class="flex-1 flex justify-center items-center gap-1.5 py-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-xl text-sm font-medium transition-colors"
                            >
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <button
                                ...
                                class="w-10 h-10 flex border items-center justify-center text-gray-400 hover:text-red-500 border-gray-200 hover:border-red-200 hover:bg-red-50 rounded-xl transition-colors"
            ...
            <!-- Empty State -->
            <div v-else class="col-span-full py-16 text-center bg-white rounded-2xl shadow-sm border border-gray-100">
                <div class="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-blue-100 shadow-inner">
                    <svg class="w-8 h-8 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                    </svg>
                </div>
                <h3 class="text-xl font-bold text-gray-900 mb-2">No services yet</h3>
                <p class="text-gray-500 mb-6 max-w-sm mx-auto">Start by adding your first service to attract customers and manage your business directly from this dashboard.</p>
                <Link
                    :href="route('vendor.services.create')"
                    class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white font-medium text-sm rounded-xl hover:bg-blue-700 hover:shadow-lg hover:-translate-y-0.5 transition-all"
                >
